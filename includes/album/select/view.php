...
{
    $sql = "select max(project_id) as 'project_id' from " . DB_NAME . ".album group by project_id";
    $rows = getFetchArray($sql);
    if ($rows != null) {
        $data = "";
        foreach ($rows as $result) {
            $name_sql="select concat(first_name, ' ', last_name) as 'name' from ".DB_NAME.".projects where id=".$result["project_id"];
            $name = getSingleValue($name_sql);
            $data .= '<li class="list-group-item">
                <a target="_blank" href="projects.php?pid='.$result["project_id"].'">'.$name.'</a>'.__images($result["project_id"]).'
            </li>';
        }
        return '¨
...
function __images($id){
    $sql = "select path from " . DB_NAME . ".album where project_id=".$id;
    $rows = getFetchArray($sql);
    if ($rows != null) {
        $data="";
        foreach ($rows as $result) {
...
<body id="page-top">
    ...
    <div id="wrapper">
        ...
        <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
            ...
            <li class="nav-item">
                <a class="nav-link" target="_blank" href="projects.php?letter_pad">
            ...
            <li class="nav-item">
                ...
                <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionSidebar">
                    <div class="bg-white py-2 collapse-inner rounded">
                        <a class="collapse-item" href="#">Monat-Berichte</a>
                        <a class="collapse-item" href="#">Jahr-Berichte</a>
                    </div>
                </div>
            </li>
